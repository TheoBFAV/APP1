...
def trouveLettre(carac, decalage, casse="min") :
    ...
    for c in alphabet :
        ...
        if carac.lower() == c :
            ...
            if alphabet.index(c) + decalage >= len(alphabet) :

                 neo_carac = alphabet[(alphabet.index(c) + decalage) - len(alphabet)]

            else :

                 neo_carac = alphabet[alphabet.index(c) + decalage]
...
def runPlanB():
    ...
    decalage=-ord(list(reponse)[17])+ord("C")
